...
public class Sorts {
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<T>> insertionSort(T[] arr) {
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            // Find the correct position by comparing with elements to the left
            while (j >= 0) {
                lst.add(new CompareEvent<>(j, i));
                if (arr[j].compareTo(curVal) > 0) {
                    // Shift element to the right
                    lst.add(new CopyEvent<>(j, j + 1, arr[j]));
                    arr[j + 1] = arr[j];
                    j--;
                } else {
                    break;
            ...
            if (j + 1 != i) {
                lst.add(new CopyEvent<>(i, j + 1, curVal));
                arr[j + 1] = curVal;
            }
        }
        return lst;
    ...
    public static <T extends Comparable<? super T>> void merge(T[] arr, int b1, int e1, int b2, int e2, List<SortEvent<T>> lst) {
        ...
        int len1 = e1 - b1;
        int len2 = e2 - b2;
        int mergedSize = len1 + len2;
        
        // Copy only the two ranges being merged into a temporary array
        @SuppressWarnings("unchecked")
        T[] temp = (T[]) new Comparable[mergedSize];
        int tempIdx = 0;
        
        // Copy first range (b1 to e1)
        for (int i = b1; i < e1; i++) {
            temp[tempIdx++] = arr[i];
        }
        
        // Copy second range (b2 to e2)
        for (int i = b2; i < e2; i++) {
            temp[tempIdx++] = arr[i];
        }
        
        // Now merge from temp back into arr starting at b1
        int i1 = 0;  // index in temp for first range
        int i2 = len1;  // index in temp for second range (starts after first range)
        int dest = b1;  // destination index in arr
        
        while (i1 < len1 && i2 < mergedSize) {
            int origIdx1 = b1 + i1;
            int origIdx2 = b2 + (i2 - len1);
            lst.add(new CompareEvent<>(origIdx1, origIdx2));
            if (temp[i1].compareTo(temp[i2]) < 0) {
                lst.add(new CopyEvent<>(origIdx1, dest, temp[i1]));
                arr[dest] = temp[i1];
                i1++;
            } else {
                lst.add(new CopyEvent<>(origIdx2, dest, temp[i2]));
                arr[dest] = temp[i2];
                i2++;
            }
            dest++;
